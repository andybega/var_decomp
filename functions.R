...
plotVarDecomp <- function(data, group, var.names=NULL, log.y=TRUE) {
	...
	p <- ggplot(data=var.df, aes(x=total, y=between.p, label=variable)) +
		geom_abline(intercept=0.5, slope=0, size=0.5, col="cyan", alpha=0.5) +
		geom_point(color="red", size=1, alpha=0.7) +
		geom_text(size=4, hjust=0, vjust=0, alpha=0.7) +
	...
	if (log.y) {
		# Minimum for x axis
		x.min <- min(var.df$total) * 0.9
		x.max <- max(var.df$total) * 1.1
		p <- p + scale_x_log10(limits=c(x.min))
	} else {
		x.min <- 0
		x.max <- max(var.df$total) * 1.1
